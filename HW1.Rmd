...
請填入以下空格完成完整偏誤論述：  
有控制**經原PR**時，擋修會使得**個經PR** (1)___(上升/下降) (2)____百分點，其值比未控制**經原PR**時還 (3)___(高/低)；這表示忽略**經原PR**會對效應係數估計產生 (4)(正向偏誤(upward bias)/負向偏誤(downward bias))。之所以會有上述的偏誤方向來自於兩個因素：

  1. **經原PR**與**個經PR**有(5)(正向/反向)關連
...
&y_i=\beta_0+\beta_1x_i+\epsilon_i\\
\\
&\bar{y_1}=\frac{\sum_iy_i}{n_1}=\frac{\sum_ix_iy_i}{\sum_ix_i},\,\bar{y_0}=\frac{\sum_iy_i}{n_0}=\frac{\sum_i(1-x_i)y_i}{\sum_i(1-x_i)}\\
\\
&\hat{\beta_1}=\frac{\sum_i(x_i-\bar{x})(y_i-\bar{y})}{\sum_i(x_i-\bar{x})^2}\\
\\
&\sum_i(x_i-\bar{x})^2=\sum_ix_i^2-\bar{x}^2,\,\sum_ix_i^2=\sum_ix_i\\
\\
&\bar{x}=\frac{\sum_ix_i}{n}=\frac{n_1}{n_0+n_1}\\
\\
&\sum_i(x_i-\bar{x})^2=n_1-(n_0+n_1)\frac{n_1^2}{(n_0+n_1)^2}=\frac{n_0n_1}{n_0+n_1}\\
\\
&\sum_i(x_i-\bar{x})(y_i-\bar{y})=\sum_i(x_i-\bar{x})y_i=\sum_ix_iy_i-\sum_i\bar{x}y_i=n_1\bar{y_1}-\frac{n_1}{n_0+n_1}(n_1\bar{y_1}+n_0\bar{y_0})\\
&\hspace{4cm}=\frac{1}{n_0+n_1}[(n_0+n_1)n_1\bar{y_1}-n_1(n_1\bar{y_1}+n_0\bar{y_0})]=\frac{n_0n_1}{n_0+n_1}(\bar{y_1}-\bar{y_0})\\
\\
&\hat{\beta_1}=\frac{\frac{n_0n_1}{n_0+n_1}(\bar{y_1}-\bar{y_0})}{\frac{n_0n_1}{n_0+n_1}}=\bar{y_1}-\bar{y_0}
\end{aligned}
